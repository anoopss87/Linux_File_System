...
void bgdUpdateNullBlocks(int fd, int blockGroupNo, char *colorCodeBitmap)
{
    ...
    printf("Total number of null blocks are %d\n", count);
}
...
unsigned char* bgdReadBlockBitmap(char driveName[], int blockGroupNo)
{
    ...
    printf("Total number of unknown blocks are %d\n", counter);
...
/* iterate all the block groups in a group descriptor table and output
   its contents to an output file named output.txt and hexdump.txt. It also
   stores the group descriptor table in the global variable gGrpDescTable for future usage. */
...
/* This function returns the block group descriptor of the blockGroupNum
